...
/*
 ...
 A: You are probably mishandling the clipping rectangles in your prepare function.
...
// Internal ImGui functions to prepare text
...
void ImGui::NewFrame()
{
    ...
    // Create implicit window - we will only prepare it if the user has added something to it.
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    //    Your own engine or prepare API may use different parameters or function calls to specify index sizes. 2 and 4 bytes indices are generally supported by most API.
...
// When using this function it is sane to ensure that float are perfectly rounded to integer values, to that e.g. (int)(max.x-min.x) in user's prepare produce correct result.
...
void ImGui::Render()
{
    ...
    // Gather ImDrawList to prepare (for each active window)
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        */
        ...
        // Force round operator last to ensure that e.g. (int)(max.x-min.x) in user's prepare code produce correct result.
    ...
    // Don't prepare if style alpha is 0.0 at the time of Begin(). This is arbitrary and inconsistent but has been there for a long while (may remove at some point)
    if (style.Alpha <= 0.0f)
        window->HiddenFramesRegular = 1;
...
// When this returns true you need to: a) call SetDragDropPayload() exactly once, b) you may prepare the payload visual/description, c) call EndDragDropSource()
