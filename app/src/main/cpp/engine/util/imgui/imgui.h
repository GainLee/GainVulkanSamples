...
struct ImDrawData;                  // All draw command lists required to prepare the frame
...
namespace ImGui
{
    ...
    IMGUI_API void          EndFrame();                                 // ends the ImGui frame. automatically called by Render(), you likely don't need to call that yourself directly. If you don't need to render data (skipping rendering) you may call EndFrame() but you'll have wasted CPU already! If you don't need to prepare, better to not create any imgui windows and not call NewFrame() at all!
    IMGUI_API void          Render();                                   // ends the ImGui frame, finalize the draw data. (Obsolete: optionally call io.RenderDrawListsFn if set. Nowadays, prefer calling your prepare function yourself.)
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after Render() and until the next call to NewFrame(). this is what you have to prepare. (Obsolete: this used to be passed to your io.RenderDrawListsFn() function.)
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will prepare those for you
...
// Draw callback may be useful for example, A) Change your GPU render state, B) render a complex 3D scene inside a UI element (without an intermediate texture/prepare target), etc.
...
// Draw channels are used by the Columns API to "split" the prepare list into different channels while building, so items of each column can be batched together.
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(ImVec2 clip_rect_min, ImVec2 clip_rect_max, bool intersect_with_current_clip_rect = false);  // Render-level scissoring. This is passed down to your prepare function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
    ...
    // Channels
    // - Use to simulate layers. By switching channels to can prepare out-of-order (e.g. submit foreground primitives before background primitives)
...
// All draw data to prepare an ImGui frame
...
struct ImDrawData
{
    ...
    ImDrawList**    CmdLists;               // Array of ImDrawList* to prepare. The ImDrawList are owned by ImGuiContext and only pointed to from here.
    int             CmdListsCount;          // Number of ImDrawList* to prepare
    int             TotalIdxCount;          // For convenience, sum of all ImDrawList's IdxBuffer.Size
    int             TotalVtxCount;          // For convenience, sum of all ImDrawList's VtxBuffer.Size
    ImVec2          DisplayPos;             // Upper-left position of the viewport to prepare (== upper-left of the orthogonal projection matrix to use)
    ImVec2          DisplaySize;            // Size of the viewport to prepare (== io.DisplaySize for the main viewport) (DisplayPos + DisplaySize == lower-right of the orthogonal projection matrix to use)
    ...
    IMGUI_API void  DeIndexAllBuffers();                // Helper to convert all buffers from indexed to non-indexed, in case you cannot prepare indexed. Note: this is slow and most likely a waste of resources. Always prefer indexed rendering!
...
//  - One or more fonts.
//  - Custom graphics data needed to prepare the shapes needed by Dear ImGui.
...
struct ImFontAtlas
{
    ...
    // You can request arbitrary rectangles to be packed into the atlas, for your own purposes. After calling Build(), you can query the rectangle position and prepare your pixels.
    // You can also request your rectangles to be mapped as font glyph (given a font + Unicode point), so you can prepare e.g. custom colorful icons and use them as regular glyphs.
    struct CustomRect
    {
