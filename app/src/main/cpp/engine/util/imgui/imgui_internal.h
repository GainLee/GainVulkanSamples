...
struct ImGuiContext
{
    ...
    // Render
    ImDrawData              DrawData;                           // Main ImDrawData instance to pass prepare information to the user
    ImDrawDataBuilder       DrawDataBuilder;
    float                   DimBgRatio;                         // 0.0..1.0 animation when fading in a dimming background (for modal window and CTRL+TAB list)
    ImDrawList              OverlayDrawList;                    // Optional software prepare of mouse cursors, if io.MouseDrawCursor is set + a few debug overlays
